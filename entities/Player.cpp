...
void Player::update_movement( std::vector<Ground*> const& grounds )
{
	...
	if ( has_powerup == true )
	{
		...
		// Changes back the speed to normal speed if we are not dashing,
		// if we are dashing, we should have the dash speed still
...
///Handles all collision between the player and Enemy objects.
void Player::handle_collisions( std::vector<Enemy*>& enemies, Button* const& button )
{
	for ( unsigned int i{0}; i < enemies.size(); ++i )
	{
		// Try to cast the enemy and bullet to a Shooting_Enemy since different things happens with different collisions.
		Shooting_Enemy* shooting_enemy = dynamic_cast<Shooting_Enemy*>(enemies[i]);
		Bullet* bullet = dynamic_cast<Bullet*>(enemies[i]);

		// If we are colliding with a shooting enemy while dashing we should get a powerup. Else we die
		...
		// If we are colliding with a bullet while dashing, the bullet should die, else we die
		...
		// The enemy object is a lava object or the wall of death
		else
		{
...
///Looks for x and y intersection, returns true if both are true.
...
/// Used when we check for intersection between a ground(in this case ceiling) and the characters head.
...
/// Used when we check for intersection between a ground and the characters feet.
...
///Updates the characters vertical speed(y_speed).
 ...
 /// Makes the character jump. Plays a jump sound effect.
void Player::jump()
{
	if ( grounded == true )
	{
...
/// Sets the dash-mode to true and changes the x_speed, plays a dash sound effect.
void Player::dash()
{
...
/// Checks if we are colliding with a button.
bool Player::button_pushed( Button* const& button ) const
{
	return intersect( button, 0 );
}

/// Draws the player texture. Calls update_animation to update its animation correct.
